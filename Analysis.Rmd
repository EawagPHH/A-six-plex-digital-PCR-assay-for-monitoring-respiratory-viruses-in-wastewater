...
This notebook generates summary statistics and plots for the manuscript “A six-plex digital PCR assay for monitoring respiratory viruses in Swiss wastewater”, which describes the development of the RESPV6 assay and its first year of implementation. Data is taken directly from the WISEdb (<https://wisedb.ethz.ch>), the Federal Office of Public Health in Switzerland (FOPH) public API (<https://api.idd.bag.admin.ch/swagger-ui/api-doc.html>) and from files shared directly from the FOPH . To run this locally you need to have an access token for the WISEdb API and the case data shared from the FOPH. Requests for a WISEdb API token can be made to the Pathogens and Human Health group at Eawag, case data must be requested from the FOPH directly.

```{r libraries}
# First three lines should only need to be run once first time you use this code
...
```{r dpcr experiment import}
comparison_raw <- read_xlsx("data/sixplex-singleplex-comparison.xlsx", sheet = "Results")
```
...
TOTAL_DROPLETS_THRESHOLD <- 15000
POSITIVE_DROPLETS_THRESHOLD <- 3

# Tidy data
comparison_all <- comparison_raw %>%
  select(
    ChamberID,
    SampleName, 
    TotalNumberOfDroplets,
    ends_with("_Concentration"),
    ends_with("_NumberOfPositiveDroplets")
  ) %>%
  rename(
    chamber_id = ChamberID,
    sample_name = SampleName,
    total_droplets = TotalNumberOfDroplets
  ) %>%
  pivot_longer(
    cols = c(ends_with("_Concentration"), ends_with("_NumberOfPositiveDroplets")),
    names_to = c("target", ".value"),
    names_pattern = "(.*)_(Concentration|NumberOfPositiveDroplets)",
  ) %>%
  rename(
    concentration = Concentration,
    positive_droplets = NumberOfPositiveDroplets
  ) %>%
  group_by(sample_name, target) %>%
  mutate(replicate = rank(chamber_id, ties.method = "random")) %>%
  ungroup() %>%
  mutate(
    assay = str_extract(sample_name, "[^_]+"),
    dilution = str_extract(sample_name, "(?<=_)[0-9:]+"),
    type = ifelse(
      grepl(pattern = "NTC", sample_name),
      "NTC",
      "sample"
    )
  )
...
# NTC data
comparison_ntc <- comparison_all %>%
  filter(
    type == "NTC",
    total_droplets >= TOTAL_DROPLETS_THRESHOLD
  ) %>%
  group_by(assay, target) %>%
  summarise(
    max_positive_droplets_ntc = max(positive_droplets),
    .groups = "drop"
  )

# Sample data
comparison_sample <- comparison_all %>%
  filter(
    type == "sample",
    (assay == "RESPV6") | (str_detect(target, assay))
  ) %>%
  left_join(
    comparison_ntc,
    by = join_by(assay, target)
  ) %>%
  mutate(
    qc_passed = (total_droplets >= TOTAL_DROPLETS_THRESHOLD) & (max_positive_droplets_ntc < 3),
    assay = ifelse(
      assay %in% c("IAV", "IBV", "MHV", "RSV", "SARS-N1", "SARS-N2"), 
      "singleplex",
      assay
    ),
    across(c(assay, dilution, replicate), as.factor)
  ) %>%
  select(assay, dilution, target, replicate, concentration, qc_passed) 

multi.single.plot.data <- comparison_sample %>%
  filter(qc_passed) %>%
  pivot_wider(
    id_cols = c(dilution, target, replicate),
    names_from = assay,
...
```{r lod experiment import}
lod_raw <- read_xlsx("data/RESPV6-lod-experiment.xlsx", sheet = "Results")
```

### Data modifications

```{r}
# Define droplet detection threshold
DETECTION_THRESHOLD <- 3

lod_data <- lod_raw %>% 
  pivot_longer(
    cols = ends_with("Concentration") | ends_with("NumberOfPositiveDroplets"),
    names_to = c("target", ".value"),
    names_sep = "_(?=[^_]+$)"
  ) %>%
  rename(
  ...
  mutate(
    detect = ifelse(positive_droplets >= DETECTION_THRESHOLD, 1, 0),
    target_conc = case_when(
      as.numeric(sample_name) == 6 ~ 31.25,
      as.numeric(sample_name) == 3 ~ 15.63,
      as.numeric(sample_name) == 1.5 ~ 7.81,
      TRUE ~ NA_real_
    ),
    # if a sample has <3 droplets, then concentration is 0
    gc_ul_rxn = ifelse(positive_droplets >= 3, concentration, 0),
    gc_rxn = gc_ul_rxn * 25
  ) %>%
  filter(sample_name != "NTC")

lod_summary <- lod_data %>%
  group_by(target, target_conc) %>%
  summarise(
    avg_gc_rxn = mean(gc_rxn, na.rm = TRUE),
    detects = sum(detect),
    non_detects = n() - sum(detect),
    detect_ratio = mean(detect),
    .groups = "drop"
  ) 
```
...
```{r abundance experiment import}
abundance_raw_1 <- read_xlsx("data/low-abundant-targets-experiment-1.xlsx", sheet = "Results")
abundance_raw_2 <- read_xlsx("data/low-abundant-targets-experiment-2.xlsx", sheet = "Results")
```
...
abundance_raw <- bind_rows(abundance_raw_1, abundance_raw_2) %>%
  mutate(SampleName = ifelse(SampleName == "low", "Low", SampleName)) 

# Tidy data
abundance_all <- abundance_raw %>%
  select(
    ChamberID,
    SampleName, 
    TotalNumberOfDroplets,
  ...
  group_by(sample_name, target) %>%
  mutate(replicate = rank(chamber_id, ties.method = "random"),
         concentration = ifelse(positive_droplets < POSITIVE_DROPLETS_THRESHOLD,
                                0,
                                concentration)) %>%
  ungroup()

abundance.plot.data <- abundance_all %>%
  mutate(spiked = ifelse(sample_name == sub("-.*", "", abundance_all$target), 
                         TRUE, FALSE),
         sample_name = factor(sample_name, 
                              levels = c("NTC", "Low", "IAV", 
                                         "IBV", "MHV", "RSV", "SARS"))) %>%
  filter(sample_name != "NTC")
```
...
readRenviron(".env")
api <- "https://wisedb.ethz.ch/api/private/"
...
wwtp_url <- paste0(api, "metadata/wastewater_treatment_plant/?reporting=True")
...
PCRdata <- get_data(pcr_url, token, "text/csv") %>%
  ...
  mutate(collection_date = ymd(collection_date),
         ...
         city = case_match(wastewater_treatment_plant.name, !!!wwtp_cities)) 

Spikeddata <- get_data(spiked_url, token, "text/csv") %>%
  mutate(collection_date = ymd(collection_date)) 

controls_from <- min(ymd_hms(PCRdata$run_timestamp))
controls_to <- max(ymd_hms(PCRdata$run_timestamp))

Controldata <- get_data(controls_url, token, "text/csv") %>%
  mutate(run_timestamp = ymd_hms(run_timestamp)) %>%
  filter(between(run_timestamp, controls_from, controls_to))
...
```{r}
dailySARScantonal <- read_csv("data/2024-08-19_covid_tests_cases_ktn_daily.csv", show_col_types = FALSE) %>%
...
```{r}
dailyIAVcantonal <- read_csv("data/2024-08-19_influA_cases_ktn_daily.csv", show_col_types = FALSE) %>%
  rename(collection_date = date,
         canton_name = canton,
         canton = canton_abbr) %>%
  mutate(collection_date = ymd(collection_date)) %>%
...
```{r}
dailyIBVcantonal <- read_csv("data/2024-08-19_influB_cases_ktn_daily.csv", show_col_types = FALSE) %>%
  rename(collection_date = date,
         canton_name = canton,
         canton = canton_abbr) %>%
  mutate(collection_date = ymd(collection_date)) %>%
...
```{r}
dailySARScatchment <- read_csv("data/2024-08-19_covid_tests_cases_ara_daily.csv", show_col_types = FALSE) %>%
  rename(collection_date = date) %>%
  mutate(collection_date = ymd(collection_date)) %>%
...
```{r ww characteristics import}
ww_char_raw <- read_xlsx("data/additional-ww-characteristics.xlsx")
```

### Data modifications

```{r}
ww_char <- df_inhibition %>%
  group_by(wastewater_treatment_plant.name, city) %>%
  summarise(avg_inhibition = mean(inhibition, na.rm = TRUE),
            .groups = "drop") %>%
  left_join(ww_char_raw %>%
              rename(wastewater_treatment_plant.name = ara_name)) %>%
  mutate(cod = as.numeric(cod),
         text_max = ifelse(avg_inhibition == max(avg_inhibition),
                           city,
                           NA),
         text_min = ifelse(avg_inhibition == min(avg_inhibition),
                           city,
                           NA))
```
...
pathogen_letters <- c("SARS" = "D",
                      "IAV" = "A", "IBV" = "B", 
                      "RSV" = "C")

target_letters_mhv <- c("SARS-N1" = "E", "SARS-N2" = "F", 
                        "IAV-M" = "A", "IBV-M" = "B", 
                        "RSV-N" = "D", "MHV" = "C")

# define colour palettes "#0000FF", "#009999"
target_colors <- c("#0000AC", "#00E3DE", "#E41A1C", "#006633", "#FA9600")
target_colors <- setNames(target_colors, c("SARS-N1", "SARS-N2", "IAV-M", "IBV-M", "RSV-N"))

pathogen_colors <- c("#0000AC", "#E41A1C", "#006633", "#FA9600")
pathogen_colors <- setNames(pathogen_colors, c("SARS", "IAV", "IBV", "RSV"))

target_full_spec_colors <- c("#0000AC", "#00E3DE", "#E41A1C", "#006633", "#FA9600")
target_full_spec_colors <- setNames(target_full_spec_colors, 
                                    c("SARS-CoV-2 (N1)", "SARS-CoV-2 (N2)",  
                                      "Influenza A", "Influenza B", 
                                      "Respiratory Syncytial Virus"))

target_full_spec_colors_mhv <- c("#0000AC", "#00E3DE", "#E41A1C", "#006633", "#FA9600", "#9D0162")
target_full_spec_colors_mhv <- setNames(target_full_spec_colors_mhv, 
                                    c("SARS-CoV-2 (N1)", "SARS-CoV-2 (N2)",  
                                      "Influenza A", "Influenza B", 
                                      "Respiratory Syncytial Virus", "Murine Hepatitus Virus"))
...
```{r}
multi.single.lm <- multi.single.plot.data %>%
  group_by(target) %>%
  do({
    model = lm(concentration_RESPV6 ~ concentration_singleplex, 
               data = mutate(., across(.cols = c(concentration_RESPV6, concentration_singleplex), 
                                       .fns = log10)))
    ...
    prediction = predict(model, newdata = group_by(., target, dilution) %>%
                           summarise(concentration_RESPV6 = mean(concentration_RESPV6),
                                     concentration_singleplex = mean(concentration_singleplex)), 
                         interval = "confidence")
  ...
  mutate(dilution = as.factor(rep(c("1", "1:10", "1:100")))) %>%
  bind_cols(group_by(multi.single.plot.data, target, dilution) %>%
              summarise(mean_concentration_singleplex = mean(concentration_singleplex)) %>%
              ungroup() %>%
              select(mean_concentration_singleplex))
```
...
```{r}
# plot data
multi.single.plot <- multi.single.plot.data %>%
  left_join(multi.single.lm) %>%
  mutate(title = target_full_names_spec[target],
         letter = target_letters_mhv[target],
         title = ifelse(
           target == "MHV",
           "Murine Hepatitus Virus",
           title)) %>%
  group_by(target) %>%
  do(p = plot_ly(data = .,
                 color = ~title,
                 colors = target_full_spec_colors_mhv,
                 showlegend = FALSE) %>%
       add_trace(x = ~concentration_singleplex,
                 y = ~concentration_RESPV6,
                 type = "scatter",
                 mode = "markers",
                 marker = list(size = 5,
                               symbol = "circle",
                               line = list(width = 1),
                               color = "white")) %>%
       add_lines(x = ~mean_concentration_singleplex,
                 y = ~fit,
                 color = ~unique(title),
                 type = "scatter",
                 mode = "lines",
                 line = list(dash = "solid")) %>%
       add_ribbons(x = ~mean_concentration_singleplex,
                   ymin = ~lwr, 
                   ymax = ~upr,
       ...
       add_annotations(text = ~unique(title),
                       ...
                       showarrow = FALSE,
                       font = list(size = 12)) %>%
       add_text(text = ~paste0(unique(equation), "<br>", unique(r2)),
                x = 10,
                y = 100,
                color = ~title,
                yref = "paper",
       ...
       layout(title = "",
              ...
              shapes = list(list(
                type = "line",
                x0 = 1,
                x1 = ~max(concentration_RESPV6),
                xref = "x",
                y0 = 1,
                y1 = ~max(concentration_RESPV6),
                yref = "y",
  ...
  layout(annotations = list(list(x = -0.15 , 
                                 y = 0.5, 
                                 text = "Concentration from six-plex [gc/µL]",
                            ...
                            list(x = 0.5, 
                                 y = -0.05, 
                                 text = "Concentration from single-plex [gc/µL]",
                                 font = list(color = "black",
                                             size = 14),
                                 textangle = 0,
                                 showarrow = F, 
...
save_image(p = multi.single.plot, file = "figures/Figure2.png", width = 750, height = 1000, scale = 5)
```

### ANOVA

```{r}
for (t in unique(multi.single.plot.data$target)) {
  cat(paste0("ANOVA for " , t, ": \n"))
  comparison_sample_filtered <- comparison_sample %>% 
    filter(target == t)
  
  fit_target <- lm(
    log10(concentration) ~ assay + dilution,
    comparison_sample_filtered
  )
  # print(summary(fit_target))
  print(anova(fit_target))
  
  # Check assumptions:
  car::qqPlot(fit_target)
  plot(fit_target, which = 1)
...
lod_data %>%
  filter(sample_name != "NTC") %>%
  ggplot(aes(x = factor(target_conc), 
             y = gc_ul_rxn*25)) +
  ...
  theme_bw() +
  labs(
    x = "Target Concentration [gc/rxn]",
    y = "Measured Concentration [gc/rxn]"
  )
```
...
```{r}
lod_probit <- lod_summary %>% 
  group_by(target) %>%
  do({
    model = glm(formula = detect_ratio ~ avg_gc_rxn,
                family = binomial(link = "probit"),
                data = .)
    
    # Extract coefficients and calculate LoD
    intercept = coef(model)["(Intercept)"]
    slope = coef(model)[[2]] 
    lod_95 = (qnorm(0.95) - intercept) / slope
    
    cat("Estimated LoD (95% detection):", lod_95, "\n")
    
    # Generate predictions
    x_vals = seq(0, 50, by = 0.1)
    newdata = data.frame(x = x_vals)
    names(newdata) = "avg_gc_rxn"
    
    prediction = predict(model, newdata = newdata, type = "response")
    
    output <- as.data.frame(prediction,
                            col.names = "prediction") %>%
      mutate(lod_95 = lod_95,
             input_pred = seq(0, 50, by = 0.1))
  })
```
...
```{r}
lod.plot <- lod_probit %>%
  left_join(select(lod_summary, c(target, avg_gc_rxn, detect_ratio))) %>%
  mutate(title = target_full_names_spec[target],
         letter = target_letters_mhv[target],
         title = ifelse(
           target == "MHV",
  ...
  do(p = plot_ly(data = .,
       ...
       add_segments(x = 0,
                    ...
                    line = list(color = "lightgrey",
                                dash = "dash",
                                width = 0.5)) %>%
       add_text(text = ~unique(round(lod_95, 2)),
                x = ~lod_95,
                y = 0.25,
                color = ~title,
                showarrow = FALSE,
  ...
  layout(annotations = list(list(x = -0.1 , 
                            ...
                            list(x = 0.5, 
                                 y = -0.05, 
                                 text = "Mean Concentration [gc/rxn]",
                                 font = list(color = "black",
                                             size = 14),
                                 textangle = 0,
                                 showarrow = F, 
...
save_image(p = lod.plot, file = "figures/SupFigure1.png", width = 750, height = 1000, scale = 5)
```

## Low abundant targets analysis

### Plot

```{r}
# plot data
abundance.plot <- abundance.plot.data %>%
  mutate(title = target_full_names_spec[target],
         letter = target_letters_mhv[target],
         title = ifelse(
           target == "MHV",
...
save_image(p = abundance.plot, file = "figures/SupFigure2.png", width = 750, height = 750, scale = 5)
```

### ANOVA

```{r}
for (t in unique(abundance.plot.data$target)) {
  cat(paste0("ANOVA for " , t, ": \n"))
  abundance.plot.data.filtered <- abundance.plot.data %>% 
    filter(target == t)
  
  anova_result <- aov(concentration ~ sample_name, 
                    data = abundance.plot.data.filtered)
...
```{r}
# Database issue: 10 chamber IDs both passed and failed - raw data checked and these chambers should have passed, therefore they are removed from fail counts
double_data <- PCRdata %>%
  filter(passed == "True") %>%
  distinct(chamber_id) %>%
  inner_join(filter(PCRdata, passed == "False")) %>%
  distinct(chamber_id) %>%
  pull(chamber_id)
...
normal_failed <- PCRdata %>%
  filter(passed == "False") %>%
  filter(!chamber_id %in% double_data) %>%
  distinct(chamber_id) %>%
  count()
...
Percentage of replicates above/below inhibition threshold

```{r}
PCRdata %>%
  filter(target == "SARS-N1") %>%
  group_by(wastewater_treatment_plant.name, collection_date) %>%
  summarise(pass = ifelse(mean(inhibition) < 0.4, TRUE, FALSE)) %>%
  ungroup() %>%
  drop_na(pass) %>%
  summarise(percent = sum(pass)/n())
```
             ...
             capture.output(
               as.data.frame(
                 ...
                 mutate(significance = cut(P.adj, breaks = c(-Inf, 0.001, 0.01, 0.05, Inf), 
                                           labels = c("***", "**", "*", ""))) %>%
                 print() %>%
                 write.csv(., "tables/SupTable2.csv", row.names = FALSE)
             )
           },
             ...
             capture.output(
               as.data.frame(
                 ...
                 mutate(significance = cut(P.adj, breaks = c(-Inf, 0.001, 0.01, 0.05, Inf), 
                                           labels = c("***", "**", "*", ""))) %>%
                 print() %>%
                 write.csv(., "tables/SupTable3.csv", row.names = FALSE))
           },
           "Kruskal-Wallis test is not significant; no need for Dunn's test." ) }
```
...
cv.plot <- PCRdata %>%
  ...
  group_by(wastewater_treatment_plant.name, 
           ...
           city,
           target, collection_date) %>%
  summarise(cv = sd(gc_per_ul, na.rm = TRUE) / mean(gc_per_ul, na.rm = TRUE) * 100,
            concentration = mean(gc_per_ul, na.rm = TRUE)) %>%
  mutate(title = target_full_names_spec[target],
         letter = target_letters_mhv[target],
         title = ifelse(
           target == "MHV",
  ...
  mutate(limit = quantile(concentration, 0.997, na.rm = TRUE),
         fit = predict(loess(cv ~ concentration, 
                             span = 0.5, 
  ...
  do(p = plot_ly(data = .,
       ...
       add_annotations(text = ~unique(letter),
                       ...
                       font = list(size = 12,
                                   weight = "bold")) %>%
       layout(xaxis = list(range = ~c(0, unique(limit))))) %>% 
  subplot(nrows = 3,
          margin = 0.05,
          widths = c(1/2, 1/2),
          heights = c(1/3, 1/3, 1/3),
  ...
  layout(annotations = list(list(x = -0.15 , 
                                 y = 0.5, 
                                 text = "CV of technical replicates",
                                 font = list(color = "black",
                                             size = 14),
                                 textangle = 270,
                                 showarrow = F, 
...
save_image(p = cv.plot, file = "figures/ReviewFigure1.png", width = 750, height = 750, scale = 5)
...
```{r}
model_A = lm(cod ~ avg_inhibition,
             data = ww_char,
             na.action = "na.exclude")
model_B = lm(cod ~ flow_per_capita,
             data = ww_char,
             na.action = "na.exclude")
model_C = lm(proportion_industry ~ avg_inhibition,
             data = ww_char,
             na.action = "na.exclude")

# Extract regression parameters
intercept_A = summary(model_A)$coefficients[1, 1]
slope_A = summary(model_A)$coefficients[2, 1]
r_squared_A = summary(model_A)$r.squared
equation_A = paste0("y = ", round(intercept_A, 2), " + ", round(slope_A, 2), "x")
r2_A = paste0("R² = ", round(r_squared_A, 2))

intercept_B = summary(model_B)$coefficients[1, 1]
slope_B = summary(model_B)$coefficients[2, 1]
r_squared_B = summary(model_B)$r.squared
equation_B = paste0("y = ", round(intercept_B, 2), " + ", round(slope_B, 2), "x")
r2_B = paste0("R² = ", round(r_squared_B, 2))

intercept_C = summary(model_C)$coefficients[1, 1]
slope_C = summary(model_C)$coefficients[2, 1]
r_squared_C = summary(model_C)$r.squared
equation_C = paste0("y = ", round(intercept_C, 2), " + ", round(slope_C, 2), "x")
r2_C = paste0("R² = ", round(r_squared_C, 2))

# Predict confidence intervals
prediction_A = predict(model_A)
prediction_B = predict(model_B)
prediction_C = predict(model_C)

ww_char_lm <- ww_char %>%
  bind_cols(fit_A = prediction_A,
            fit_B = prediction_B,
            fit_C = prediction_C)
```
...
```{r}
cod_inhib <- plot_ly(ww_char_lm,
                     showlegend = FALSE) %>%
  add_trace(x = ~avg_inhibition,
            y = ~cod,
            type = "scatter",
            mode = "markers",
            marker = list(color = "black")) %>%
  add_text(x = ~avg_inhibition,
           y = ~cod,
           text = ~text_max,
  ...
  add_annotations(text = "A",
                  ...
                  font = list(size = 12,
                              weight = "bold")) %>%
  layout(xaxis = list(title = "Average Inhibition [%]",
                      zeroline = FALSE),
         yaxis = list(title = "Chemical Oxygen Demand [mg/L]"),
         annotations = list(
           list(
             x = 0.1,
             y = 1,
             ...
             xanchor = "left",
             yanchor = "top",
             text = paste(equation_A, "<br>", r2_A),
             showarrow = FALSE,
             font = list(size = 10, color = "blue")
           )))

cod_flow <- plot_ly(ww_char_lm,
                    showlegend = FALSE) %>%
  add_trace(x = ~flow_per_capita,
            y = ~cod,
            type = "scatter",
            mode = "markers",
            marker = list(color = "black")) %>%
  add_text(x = ~flow_per_capita,
           y = ~cod,
           text = ~text_max,
  ...
  add_annotations(text = "B",
                  ...
                  font = list(size = 12,
                              weight = "bold")) %>%
  layout(xaxis = list(title = "Flow per Capita [L person<sup>-1</sup>]",
                      zeroline = FALSE),
         yaxis = list(title = "Chemical Oxygen Demand [mg/L]"),
         annotations = list(
           list(
             ...
             xanchor = "right",
             yanchor = "top",
             text = paste(equation_B, "<br>", r2_B),
             showarrow = FALSE,
             font = list(size = 10, color = "blue")
           )))

industry_inhib <- plot_ly(ww_char_lm,
                          showlegend = FALSE) %>%
  add_trace(x = ~avg_inhibition,
            y = ~proportion_industry,
            type = "scatter",
            mode = "markers",
            marker = list(color = "black")) %>%
  add_text(x = ~avg_inhibition,
  ...
  layout(xaxis = list(title = "Average Inhibition [%]",
                      zeroline = FALSE),
         yaxis = list(title = "Industry Proportion [%]"),
         annotations = list(
           list(
             x = 0.1,
             y = 1,
             ...
             xanchor = "left",
             yanchor = "top",
             text = paste(equation_C, "<br>", r2_C),
             showarrow = FALSE,
             font = list(size = 10, color = "blue")
           )))


ww_char_plot <- subplot(cod_inhib, cod_flow, industry_inhib,
                        nrows = 1,
                        margin = 0.05,
...
save_image(p = ww_char_plot, file = "figures/SupFigureText1.png", width = 750, height = 300, scale = 5)
```
...
save_image(p = N1.N2.plot, file = "figures/Figure3.png", width = 750, height = 750, scale = 5)
...
save_image(p = N1.N2.ratio.plot, file = "figures/SupFigure3.png", width = 750, height = 1000, scale = 5)
...
N1.N2.ratio.plot.data %>%
  ...
  print() %>%
  write_csv(., "tables/SupTable7-FigS3.csv")
```
...
save_image(p = sars.loads.plot, file = "figures/Figure4.png", width = 750, height = 1000, scale = 5)
...
avgData %>%
  ...
  print() %>%
  write_csv(., "tables/SupTable7-Fig4.csv")
```
...
save_image(p = flu.loads.plot, file = "figures/Figure5.png", width = 750, height = 1000, scale = 5)
...
avgData %>%
  ...
  select(city, target, collection_date, load) %>%
  print() %>%
  write_csv(., "tables/SupTable7-Fig5.csv")
```
...
save_image(p = wwtps.loads.plot, file = "figures/SupFigure4.png", width = 750, height = 750, scale = 5)
...
for(i in 1:length(cases.report.loads.plot)) {
  ...
  save_image(p = cases.report.loads.plot[[i]],
             file = paste0("figures/SupFigure5-", unique(swissAvgData$pathogen)[i], ".png"),
             width = 500, height = 375, scale = 5)
}
...
for (i in 1:length(unique(swissAvgData$pathogen))) {
  ...
  swissAvgData %>%
    ...
    print() %>%
    write.csv(., paste0("tables/SupTable7-FigS5-", unique(.$pathogen), ".csv"))
}
...
cantonDataWeek %>%
  ...
  arrange(city) %>%
  print() %>%
  write.csv(., "tables/SupTable4-weekly.csv", row.names = FALSE)
...
sentinella.loads.legend <- plot_ly(data = filter(swissAvgData, pathogen == "SARS"),
                                   x = ~collection_date) %>%
  add_trace(y = ~median_load,
            name = "Median Daily Viral RNA Load",
            yaxis = "y", 
...
for(i in 1:length(sentinella.loads.plot)) {
  save_image(p = sentinella.loads.plot[[i]], 
             file = paste0("figures/Figure6-", unique(swissAvgData$pathogen)[i], ".png"), 
             width = 500, height = 375, scale = 5)
}

save_image(p = sentinella.loads.legend, file = "figures/Figure6-legend.png", width = 1000, height = 750, scale = 5)
...
for (i in 1:length(unique(swissAvgData$pathogen))) {
  swissAvgData %>%
    ...
    filter(median_load > plyr::round_any(max(load_median), 10000000, f = ceiling)) %>%
    print() %>%
    write.csv(., paste0("tables/SupTable7-Fig6-", unique(.$pathogen), ".csv"))
}
...
swissAvgDataWeek %>%
  ...
  {. ->> SentinellaTimeLag } %>%
  print() %>%
  write_csv(., "tables/SupTable5.csv")
```
...
save_image(p = sentinella.lags, file = "figures/SupFigure6.png", width = 750, height = 1000, scale = 5)
...
dailySARScantonal %>%
  ...
  arrange(city) %>%
  print() %>%
  write.csv(., "tables/SupTable4-daily.csv", row.names = FALSE)
